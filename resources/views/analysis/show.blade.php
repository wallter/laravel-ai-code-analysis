...
@php
    ...
    $staticAnalyses = $analysis->staticAnalyses->sortByDesc('created_at');
@endphp
...
{{ $rawContent }}
  ...
  @if(!$staticAnalyses->isEmpty())
    <div x-data="{ expandAllStatic: false }" class="space-y-4">
      ...
      <div class="space-y-6">
        @foreach($staticAnalyses as $staticAnalysis)
          @php
            $analysisName = $staticAnalysis->name;
            $analysisCost = $staticAnalysis->cost ?? 0;
            $analysisData = $staticAnalysis->data ?? '';

            $costClasses = $analysisCost > 0.01
                ? 'bg-yellow-100 text-yellow-800'
                : 'bg-green-100 text-green-800';
            
            // Open the latest static analysis by default
          ...
          >
                ...
                <h4 class="text-base font-semibold dark:text-gray-100 flex items-center">
                  ...
                  >
                    ...
                    <div
                      ...
                      style="display: none;"
                    >
                      Detailed information about the static analysis run.
                    </div>
                  </span>
                </h4>
                <span class="text-xs text-gray-500 dark:text-gray-400">
            ...
            >
              ...
              >
                <span x-show="!viewRawStatic">Show JSON</span>
...
{!! json_encode(json_decode($analysisData, true), JSON_PRETTY_PRINT) !!}
                  </pre>
                </div>
              </div>
